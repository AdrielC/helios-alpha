...
function openAlertEntity(alert: AlertView): void {
  ...
  if (["strategy", "stage", "control", "account"].includes(related.kind)) {
    view.value = "control";
    return;
  }
  view.value = "operations";
  let section = "telemetry";
  if (related.kind === "signal") { selectedSignalId.value = related.id; section = "signals"; }
  else if (related.kind === "source") section = "sources";
  else if (related.kind === "order") { inspectedOrderId.value = related.id; section = "orders"; }
  else if (related.kind === "position") section = "positions";
...
function actOnAlert(alert: AlertView): void {
  if (alert.id === "runtime-stale-view") { void connectPort(); return; }
  openAlertEntity(alert);
}
  ...
  <div class="operator-console">
    <Suspense>
      <OperatorHeader
        ...
        :data-class="snapshot.dataClass"
        :has-snapshot="hasSnapshot"
        :mode="snapshot.mode"
        :observed-at="snapshot.observedAt"
        :view="view"
        @act-alert="actOnAlert"
        @freeze="handleViewFreeze"
        @open-alert="openAlert"
        @select-view="view = $event"
      />
      <template #fallback><div class="header-fallback"><span></span><strong>Helios</strong><small>Connecting</small></div></template>
    </Suspense>
    ...
    <template v-else-if="view === 'operations'">
      ...
      <div class="operations-layout" :data-rail="railCollapsed ? 'collapsed' : 'expanded'">
        <aside class="entity-rail" aria-label="Operations sections">
          <button class="rail-toggle" :aria-label="railCollapsed ? 'Expand section navigation' : 'Collapse section navigation'" :aria-pressed="railCollapsed" @click="railCollapsed = !railCollapsed"><svg viewBox="0 0 18 18" aria-hidden="true"><path :d="railCollapsed ? 'm6.5 4.5 4.5 4.5-4.5 4.5' : 'm11.5 4.5-4.5 4.5 4.5 4.5'" /></svg><span>{{ railCollapsed ? "Expand" : "Collapse" }}</span></button>
          <a href="#telemetry" class="active" title="Telemetry"><b aria-hidden="true">TL</b><em>Telemetry</em></a>
          <a href="#positions" title="Positions"><b aria-hidden="true">PS</b><em>Positions</em><span>{{ snapshot.positions.length }}</span></a>
          <a href="#orders" title="Orders"><b aria-hidden="true">OR</b><em>Orders</em><span>{{ snapshot.orders.length }}</span></a>
          <a href="#signals" title="Signals"><b aria-hidden="true">SG</b><em>Signals</em><span>{{ snapshot.signals.length }}</span></a>
          <a href="#activity" title="Activity"><b aria-hidden="true">AC</b><em>Activity</em><span>{{ snapshot.activity.length }}</span></a>
          <a href="#sources" title="Sources"><b aria-hidden="true">SR</b><em>Sources</em><span>{{ snapshot.sources.length }}</span></a>
...
.header-fallback{display:flex;min-height:64px;gap:10px;align-items:center;padding:0 16px;border-bottom:1px solid var(--atlas-rule);background:var(--atlas-ground)}.header-fallback>span{width:25px;height:25px;border:1px solid var(--atlas-green)}.header-fallback strong{font-size:16px}.header-fallback small{color:var(--atlas-axis);font:8px var(--vp-font-family-mono);text-transform:uppercase}
...
.operations-layout{display:grid;grid-template-columns:188px minmax(0,1fr);max-width:1920px;min-height:calc(100vh - 64px);margin:0 auto;border-right:1px solid var(--atlas-rule);border-left:1px solid var(--atlas-rule);transition:grid-template-columns .24s cubic-bezier(.16,1,.3,1)}.operations-layout[data-rail=collapsed]{grid-template-columns:54px minmax(0,1fr)}.entity-rail{position:sticky;top:64px;align-self:start;min-height:calc(100vh - 64px);padding-top:7px;border-right:1px solid var(--atlas-rule);background:var(--atlas-surface-alt)}.rail-toggle{display:flex;width:100%;min-height:34px;gap:9px;align-items:center;padding:0 14px;color:var(--atlas-axis);font:8px var(--vp-font-family-mono);text-transform:uppercase;border:0;border-bottom:1px solid var(--atlas-rule-soft);background:transparent;cursor:pointer}.rail-toggle:hover{color:var(--atlas-blue);background:var(--atlas-blue-soft)}.rail-toggle svg{width:17px;flex:0 0 auto;fill:none;stroke:currentColor;stroke-width:1.4}.entity-rail>a{display:grid;grid-template-columns:24px minmax(0,1fr) auto;gap:8px;align-items:center;min-height:39px;padding:0 13px;color:var(--atlas-muted);font:10px var(--vp-font-family-mono);text-decoration:none;border-top:1px solid transparent;border-bottom:1px solid transparent}.entity-rail>a b{color:var(--atlas-axis);font:600 8px var(--vp-font-family-mono);letter-spacing:.04em}.entity-rail>a em{overflow:hidden;font-style:normal;text-overflow:ellipsis;white-space:nowrap}.entity-rail>a span{color:var(--atlas-axis);font-variant-numeric:tabular-nums}.entity-rail>a:hover,.entity-rail>a.active{color:var(--atlas-blue);border-color:var(--atlas-rule-soft);background:var(--atlas-blue-soft)}.entity-rail>a:hover b,.entity-rail>a.active b{color:var(--atlas-blue)}.operations-layout[data-rail=collapsed] .rail-toggle{padding:0 17px}.operations-layout[data-rail=collapsed] .rail-toggle span,.operations-layout[data-rail=collapsed] .entity-rail>a em,.operations-layout[data-rail=collapsed] .entity-rail>a span,.operations-layout[data-rail=collapsed] .runtime-facts{display:none}.operations-layout[data-rail=collapsed] .entity-rail>a{grid-template-columns:1fr;padding:0;text-align:center}.operations-layout[data-rail=collapsed] .entity-rail>a b{font-size:9px}.runtime-facts{position:absolute;right:0;bottom:0;left:0;padding:13px 15px;border-top:1px solid var(--atlas-rule)}.runtime-facts dl{display:grid;gap:7px;margin:0}.runtime-facts dl div{display:grid;grid-template-columns:1fr auto;gap:8px}.runtime-facts dt,.runtime-facts dd{margin:0;color:var(--atlas-axis);font:8px var(--vp-font-family-mono);text-transform:uppercase}.runtime-facts dd{overflow:hidden;max-width:92px;color:var(--atlas-muted);text-overflow:ellipsis;white-space:nowrap}.operations-workspace{min-width:0}
...
@media(max-width:1180px){.entity-rail{top:105px}}
...
@media(max-width:620px){.metric-strip dl{min-width:760px}.telemetry-section{scroll-margin-top:0}.section-header{align-items:flex-start;flex-direction:column;height:auto;min-height:60px;padding:12px}.metric-tabs{width:100%}.metric-tabs button{padding:7px 10px}.signal-inspector{padding:14px 12px}.signal-inspector>header{align-items:flex-end}.signal-inspector dl{grid-template-columns:1fr}.signal-inspector dl>div{border-right:0;border-bottom:1px solid var(--atlas-rule-soft)}.order-detail{grid-template-columns:repeat(2,1fr)}.activity-table{overflow-x:auto}.activity-table>div{min-width:650px}.alert-inspector{padding:26px 18px}.alert-inspector dl{grid-template-columns:1fr}}
