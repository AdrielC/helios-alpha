...
| **`helio_backtest`** | **Backtest harness:** `Clock` / `FixedClock` / `WallClock`, `EpochRange`, SHA-256 **pipeline fingerprint** (includes optional Kalman options JSON). **Kalman:** `KalmanLocalLevelScan` is a `helio_scan` `Scan` + `SnapshottingScan` (O(1) step, serde snapshot, composable with `then` / `run_slice`); `train_local_level_heuristic` fits `q`/`r` on a capped prefix. Native **Ratatui** TUI: `cargo run -p helio_backtest --features tui --bin helio-backtest-tui`. |
