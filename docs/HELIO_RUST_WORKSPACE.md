...
| **`helio_stats`** | **Substrate, statistics:** stable online/parallel moments and covariance, Neumaier compensation, scaled norms, log probabilities, guarded scan adapters, and an O(1) exponential `HawkesScan`. No trading types and no claim that fitted parameters imply alpha. |
...
| **`helio_backtest`** | **Backtest harness:** `Clock` / `FixedClock` / `WallClock`, `EpochRange`, SHA-256 **pipeline fingerprint** (includes optional Kalman options JSON). **Kalman:** `GuardedKalmanLocalLevelScan` is the atomic fallible streaming adapter; `KalmanLocalLevelScan` remains the compatibility adapter. Offline fit: **`fit_local_level_em`** (EM: forward filter + **RTS smoother** + M-step for `q`,`r`) or **`fit_local_level_mle`** (faster innovation MLE). `KalmanHarnessOptions.fit_mode` selects `em` vs `mle`. Native **Ratatui** TUI: `cargo run -p helio_backtest --features tui --bin helio-backtest-tui`. |
