...
- **Session authority:** shocks in the CLI path use [`merge_session_for_shock`](rust/crates/helio_event/src/event_shock_ingest.rs) with the same [`TradingCalendar`](rust/crates/helio_time/src/calendar.rs) as the vertical; [`validate_bar_sessions_vs_shock_calendar`](rust/crates/helio_event/src/event_shock_ingest.rs) only catches bars keyed to [`utc_naive_civil_day_index`](rust/crates/helio_time/src/calendar.rs) of `available_at` when that naive UTC midnight bucket is a **non-session** day but the shock rolls to a real session (e.g. weekend UTC index). It does **not** encode venue “trade date” / session-open-in-local-evening rules — those need your own `SessionDate` mapping before bars and shocks meet here.
