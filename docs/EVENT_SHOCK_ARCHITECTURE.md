...
- **Merge key:** [`vertical_merge_key`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_vertical.rs) sorts by `(session, kind, stream_seq)` with stable tie-break on ingest order; bars sort before shocks in the same session. See rustdoc on [`build_vertical_replay`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_ingest.rs) vs [`build_vertical_replay_with_calendar`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_ingest.rs).
- **Session authority:** shocks in the CLI path use [`merge_session_for_shock`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_ingest.rs) with the same [`TradingCalendar`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_time/src/calendar.rs) as the vertical; [`validate_bar_sessions_vs_shock_calendar`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_ingest.rs) only catches bars keyed to [`utc_naive_civil_day_index`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_time/src/calendar.rs) of `available_at` when that naive UTC midnight bucket is a **non-session** day but the shock rolls to a real session, such as a weekend UTC index. It does **not** encode venue “trade date” or session-open-in-local-evening rules. Those need your own `SessionDate` mapping before bars and shocks meet here.
- **Checkpoints:** [`EventShockVerticalScan::flush`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_vertical.rs) forwards `FlushReason` to all sub-scans; replay helpers document cadence in [`event_shock_replay`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_replay.rs).
- **Execution backpressure:** [`ExecutionBufferPolicy`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_execution.rs) on [`SignalExecutionScan`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_execution.rs); default unbounded; optional cap with deterministic `DropOldest`. Wire via [`EventShockVerticalScan::with_exec_buffer`](https://github.com/AdrielC/helios-alpha/blob/main/rust/crates/helio_event/src/event_shock_vertical.rs).
