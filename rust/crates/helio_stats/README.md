...
- `CompensatedSum`: Neumaier-compensated sum with mergeable, checkpointed correction state.
- `ScaledSumSquares`: LAPACK-style scaled norm and sum-of-squares state.
- `LogProbability`: conditional probability products in natural-log space.
...
The scan adapters implement `FallibleRestoreScan`, so non-finite or structurally invalid external snapshots are rejected before resume. Failed numerical updates leave live state unchanged.
...
The `bayes` Cargo feature is enabled by default. WASM consumers that only need moments, Hawkes,
and guarded numerical scans can use `default-features = false` to avoid RNG dependencies.
